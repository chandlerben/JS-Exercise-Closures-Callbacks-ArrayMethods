...
var runnersData = require('./data/runners');
...
function processSum(numberList, callback) {
  return callback(numberList.reduce((total, current) => total + current, 0))
}
...
function getFullNames(runners) {
  let arrayOfNames = []
  runners.forEach((element) => arrayOfNames.push(`${element.last_name}, ${element.first_name}`))
  return arrayOfNames
}
...
function firstNamesAllCaps(runners) {
  return runners.map((element) => element.first_name.toUpperCase())
}
...
function tallyUpDonations(runners) {
  return runners.reduce((total, b) => total + b.donation, 0)
}
